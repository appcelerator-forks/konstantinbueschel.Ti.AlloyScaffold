 ...
 *      async:                      true,
 *
 *      contentType:                Tools.getOSProperty({
 *
 ...
 * @return void
 */
function ChunkedUploader(_options) {
    ...
    this._currentChunkIdentifier = this._createChunkIdentifier(Tools.padDigits(this._currentChunkNumber, 2));
    ...
    if (Tools.type(this._options.timeout) === 'number') {

        xhrOptions = Tools.extend(xhrOptions, {
 ...
 * @return void
 */
ChunkedUploader.prototype._upload = function() {
    ...
    setTimeout(function() {
        ...
        if (chunkUploaded === true || Tools.type(that._currentChunk) === 'undefined') {
            ...
            try {
                ...
                // DEBUG ERROR
                Ti.API.error('------ DIA');
                Ti.API.error('ChunkedUploader.js - _upload() - _fileStream.read(...) Error!');
                Ti.API.error('bytesRead = ' + bytesRead);
                Ti.API.error('that._chunkBuffer = ' + that._chunkBuffer);
                Ti.API.error('_exception = ');
                Ti.API.error(_exception);
                Ti.API.error(JSON.stringify(_exception));
                Ti.API.error('------ DIA');
            }
        ...
        if (Tools.type(that._currentChunk) !== 'undefined' && chunkUploaded === false) {
 ...
 * @return void
 */
ChunkedUploader.prototype._onChunkUploadComplete = function(_event) {
 ...
 * @return void
 */
ChunkedUploader.prototype._onChunkUploadError = function(_event) {
    ...
    // DEBUG ERROR
    Ti.API.error('--------------------------- DIA');
    Ti.API.error('ChunkedUploader.js - _onChunkUploadError()');

    Ti.API.error('this._currentChunkNumber = ' + this._currentChunkNumber);
    Ti.API.error('this._totalChunkCount = ' + this._totalChunkCount);

    Ti.API.error('this._extantRetries = ' + this._extantRetries);
    Ti.API.error('this._extantRetries-- = ' + this._extantRetries--);

    Ti.API.error('result = ' + result);
    Ti.API.error('--------------------------- DIA');
    ...
    if (Tools.type(this._extantRetries) === 'number' && this._extantRetries-- > 0) {
 ...
 * @return void
 */
ChunkedUploader.prototype.pause = function() {
 ...
 * @return void
 */
ChunkedUploader.prototype.resume = function() {
 ...
 */
ChunkedUploader.prototype.getUploadClient = function() {
 ...
 * @return {String}
 */
ChunkedUploader.prototype._createChunkIdentifier = function(_prefix) {

    // load toolbox
    var Tools = require('/common/tools'),

        prefix = (Tools.type(_prefix) === 'string' && _prefix.length ? _prefix : '');

    return (prefix + '_' + Number(Tools.createRandomNumber(1, 9, 8).toString() + (Tools.getNow().toString()).substr(-2)));
 ...
 * @return {Boolean}
 */
ChunkedUploader.prototype.isUploading = function() {
 ...
 * @return {Boolean}
 */
ChunkedUploader.prototype.isFinished = function() {
 ...
 * @return void
 */
ChunkedUploader.prototype.cleanup = function() {
