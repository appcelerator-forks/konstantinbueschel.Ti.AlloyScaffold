...
Ti.API.info("[http].request():params.url = ", params.url);
	...
	xhr.onload = function(onloadEvent) {
		...
		if (result) {
            ...
            if (OS_ANDROID && params.fileHandle) {
            ...
            that.get(
                ...
                function(resultData) {
                    ...
                    if (item.doneEvent) {
	                    ...
	                    require('/app/EventDispatcher').trigger(item.doneEvent, resultData);
                    }
                ...
                function(resultData) {
                    ...
                    if (item.failEvent) {
	                    ...
	                    require('/app/EventDispatcher').trigger(item.failEvent, resultData);
                    }
...
exports.cancel = function(args) {
	...
	// DEB(function(){/*Ti.API.debug*/})('[http].cancel():', 'args?', JSON.stringify(args), 'open xhr count?', Object.keys(_xhrClients).length);
	...
	if (args) {
		...
		// DEBU(function(){/*Ti.API.debug*/})('[http].cancel():', 'requestNames', requestNames);
		...
		requestNames.forEach(function(requestName) {
			...
			if (xhr) {
				...
				try {
					...
					// DEBUG
	(function(){/*Ti.API.debug*/})('[http].cancel():Aborting xhr for request "', requestName, '"');
	...
	// DEB(function(){/*Ti.API.debug*/})('[http].cancel():END', 'open xhr count?', Object.keys(_xhrClients).length);
...
exports.cancelAll = function() {
	...
	require('/common/tools').each(_xhrClients, function(xhr) {
