...
function _defaultHandler(response) {
	...
	// DEB(function(){/*Ti.API.debug*/})('[installrUpdate]._defaultHandler(): local', currentVersion, 'installr', response.versionNumber, response.buildNumber, 'isSame =', isSame);
 ...
 * @return void
 */
function _checkForNewerVersion(appId, _success) {
	...
	http.onload = function () {
		...
		if (responseJSON.result == "success") {
			...
			// DEBUG(function(){/*Ti.API.debug*/})('[installrUpdater].http.onload():responseJSON.appData =', responseJSON.appData);
 ...
 * @return void
 */
function _autocheck(theAppId) {

	// run this on resume and network back online
	Ti.App.addEventListener('resume', function () {
		_checkForNewerVersion(theAppId);
	});
	...
	_checkForNewerVersion(theAppId, undefined);
