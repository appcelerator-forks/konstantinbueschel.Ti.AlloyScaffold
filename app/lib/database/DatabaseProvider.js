...
DatabaseProvider.prototype.isDatabaseSchemeUpToDate = function(currentDatabaseVersion) {
    ...
    // DEBUG WARNING
(function(){/*Ti.API.warn*/})('[DatabaseProvider]:Database scheme is deprecated');
    ...
    if (updateToVersion && updateToVersion > 0) {
        ...
        switch (updateToVersion) {
            ...
            default:
                ...
                // DEBUG WARNING
            (function(){/*Ti.API.warn*/})('[DatabaseProvider]:alterDatabaseSchemeToVersion():Tried to update to not specified database scheme version! (Failed version update: "', updateToVersion, '")');
...
DatabaseProvider.prototype.getDatabaseFile = function(databaseName) {
    ...
    var dbFullName = (OS_ANDROID ? databaseName : ((_helpers.endsInSQL(databaseName)) ? databaseName : databaseName + '.sql'));
...
DatabaseProvider.prototype.renameDatabase = function(oldName, newName) {
    ...
    if (db.exists()) {
        ...
        newName = (OS_ANDROID ? newName : ((_helpers.endsInSQL(newName)) ? newName : newName + '.sql'));
...
DatabaseProvider.prototype.listDatabases = function() {
    ...
    for (iLoop = 0; iLoop < iLength; iLoop++) {
        ...
        if (tempFile.exists()) {
            ...
            if (OS_ANDROID || (OS_ANDROID === false && tempFile.extension().toLowerCase() === 'sql')) {
